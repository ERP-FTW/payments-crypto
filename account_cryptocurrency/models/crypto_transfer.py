...
class CryptoTransfer(models.Model):
    ...
    provider_request_id = fields.Char(index=True, copy=False, readonly=True)
    ...
    @api.depends("line_ids", "res_currency_move_ids")
    def _compute_counts(self):
        for payout in self:
            payout.line_count = len(payout.line_ids)
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if not vals.get("name") or vals.get("name") == _("New"):
                vals["name"] = seq.next_by_code("crypto.transfer") or _("New")
            config = None
            if vals.get("config_id"):
                config = self.env["crypto.transfer.config"].browse(vals["config_id"])
                vals.setdefault("company_id", config.company_id.id)
            if vals.get("line_ids") and config:
                ...
                vals["line_ids"] = normalized_lines
    ...
    def _onchange_config_id(self):
        for payout in self:
            ...
            if currency and payout.line_ids:
        ...
        for payout in self:
            if payout.config_id.company_id != payout.company_id:
                raise ValidationError(
                    _("The transfer company must match the configuration company.")
                )
    ...
    def _ensure_withdrawals_currency(self):
        for payout in self:
            ...
            bad_lines = payout.line_ids.filtered(
                lambda w: w.currency_code and w.currency_code != expected_code
            )
            if bad_lines:
                raise UserError(
                    _(
                        "All withdrawals must use currency code %(code)s for transfer %(payout)s."
                    )
    ...
    def _get_outbound_payment_method_line(self, journal):
        ...
        if not method_line:
            raise UserError(
                _(
                    "Journal %(journal)s has no outbound payment method lines. Configure one before posting the transfer."
                )
    ...
    def action_submit_to_provider(self):
        for payout in self:
            if not self.env.user.has_group("account_cryptocurrency.group_crypto_transfer_operator"):
                raise UserError(_("Only Crypto Transfer Operators may submit transfers."))
            try:
                self.env.cr.execute("SELECT id FROM crypto_transfer WHERE id = %s FOR UPDATE NOWAIT", [payout.id])
            ...
            try:
                payout.provider_id.provider_submit_transfer(payout)
            except TimeoutError:
    ...
    def action_refresh_status(self):
        for payout in self:
            ...
            payout.provider_id.provider_get_transfer(payout)
    ...
    def _validate_before_post_outflow(self):
        ...
        if not self.env.user.has_group("account_cryptocurrency.group_crypto_transfer_accountant"):
            raise UserError(_("Only Crypto Transfer Accountants may post confirmed outflows."))
        if not self.line_ids or any(not l.verified_event_id.signature_verified or not l.executed_at or not l.executed_atomic_amount for l in self.line_ids):
        ...
        if journal.company_id != self.company_id:
            raise UserError(
                _("Crypto journal company must match the transfer company.")
            )
        if not config.transfer_partner_id:
    ...
    def action_post_crypto_outflow_payment(self):
        ...
        for payout in self:
            ...
            payment_date = fields.Date.to_date(min(payout.line_ids.mapped("executed_at")))
            ref = _("CRYPTO TRANSFER %(name)s%(provider)s") % {
                "name": payout.name,
                "provider": (
                    f" / {payout.provider_request_id}" if payout.provider_request_id else ""
                ),
            }
            payment_vals = {
                "payment_type": "outbound",
                "partner_type": "supplier",
                "partner_id": config.transfer_partner_id.id,
                "amount": amount,
            ...
            if not currency_moves:
                _logger.warning(
                    "No currency moves were created for transfer %s (payment %s).",
                    payout.display_name,
                    payment.display_name,
                )
            elif any(move.direction != "outbound" for move in currency_moves):
                _logger.warning(
                    "Currency moves for transfer %s are not all outbound: %s",
                    payout.display_name,
    ...
    def write(self, vals):
        ...
        for field_name in ("provider_batch_id", "provider_request_id"):
        ...
        editable = {"name", "config_id", "company_id", "note", "line_ids"}
    ...
    _sql_constraints = [
        ("payout_provider_idempotency_uniq", "unique(provider_id, external_idempotency_key)", "Transfer idempotency keys must be unique per provider."),
        ("payout_provider_batch_uniq", "unique(provider_id, provider_batch_id)", "This provider batch is already linked."),
    ]
